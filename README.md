...
- `apps/server`：Node.js + Hono + Drizzle API，提供健康检查、认证和系统用户、组织部门、系统角色、权限资源接口。
- `apps/client`：Vue 3 + Vite 前端，包含登录、注册、后台管理壳层，以及系统用户、组织部门、系统角色、权限资源页面，通过 `/api` 代理调用服务端。
...
- 当前前端后台管理壳层使用 Naive UI 菜单，由服务端菜单资源驱动，并支持 `v-can` 按钮级权限显示；权限资源页支持新增、编辑、新增下级、删除叶子权限资源和图标文本预览；个人设置入口位于后台侧边栏用户区域，不占用菜单资源。
