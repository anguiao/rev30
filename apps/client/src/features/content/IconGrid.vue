...
<template>
  <div class="grid grid-cols-[repeat(auto-fill,11rem)] gap-3">
    <component
      ...
      class="relative flex h-22 w-44 items-start gap-3 rounded-md border border-stone-200 bg-white p-3 text-left transition-colors hover:border-primary hover:bg-stone-50 focus-visible:border-primary focus-visible:bg-stone-50 focus-visible:outline-none dark:border-zinc-800 dark:bg-zinc-950 dark:hover:bg-zinc-900 dark:focus-visible:bg-zinc-900"
    ...
    >
      ...
      <div
        v-if="editable"
        class="absolute top-2 right-2 flex items-center gap-1 rounded-md bg-white/90 p-0.5 dark:bg-zinc-950/90"
      >
        <NTooltip trigger="hover">
          <template #trigger>
            <NButton
        ...
        <NTooltip v-if="canRename" trigger="hover">
          <template #trigger>
            ...
            >
              <span class="i-[lucide--pencil-line] text-sm" aria-hidden="true" />
            </NButton>
          </template>
          重命名
        </NTooltip>
