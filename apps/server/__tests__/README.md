...
- `helpers/system.ts`：提供最小 user、department、role 和 resource fixture；权限、关联关系及场景差异仍在用例中显式声明。
- `helpers/http.ts`：HTTP 边界测试的 JSON 请求、响应解析和状态/响应体断言工具；新增或重构路由测试时优先使用，避免重复手写 `JSON.stringify`、`content-type` 和类型断言。

PGlite 集成测试证明单连接数据库中的约束、事务结果和应用层原子结果，不代表生产 PostgreSQL 的跨连接锁、调度或隔离级别语义。
...
pnpm --filter @rev30/server coverage
```

覆盖率只用于诊断盲区，不设置百分比门槛，也不属于 `pnpm check`。HTML 报告生成在 `apps/server/coverage/`。
