...
async function createTestApp(database: TestDatabase, options: TestAppOptions = {}) {
...
describe('dictionary routes', () => {
  dbTest(
    'supports create/get/list/put/delete lifecycle for dictionaries',
    async ({ db: database }) => {
  ...
  dbTest(
    'allows authenticated users without list access to query options',
    async ({ db: database }) => {
  ...
  dbTest(
    'counts only active items in list itemCount and keeps disabled items in detail',
    async ({ db: database }) => {
      const app = await createTestApp(database)
  ...
  dbTest('allows reusing deleted item value in same put payload', async ({ db: database }) => {
    const app = await createTestApp(database)
    const { body: created } = await createDictionary(app, {
      code: 'reusable',
      name: '可复用',
  ...
  dbTest(
    'rejects updating dictionary with item id from another dictionary',
    async ({ db: database }) => {
      const app = await createTestApp(database)
  ...
  dbTest(
    'returns duplicate active code conflict and allows reusing soft-deleted code',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: first } = await createDictionary(app, {
        code: 'dup-code',
  ...
  dbTest(
    'allows moving retained item values when final payload is unique',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: created } = await createDictionary(app, {
      ...
      expect(detailBody.items.every((item) => !item.value.startsWith('__dictionary_tmp_'))).toBe(
        true,
      )
  ...
  dbTest('allows reusing soft-deleted old value in same dictionary', async ({ db: database }) => {
    const app = await createTestApp(database)
    const { body: created } = await createDictionary(app, {
      code: 'item-value',
      name: '字典项值',
  ...
  dbTest(
    'cascades soft delete from dictionary type to its undeleted items',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: created } = await createDictionary(app, {
        code: 'cascade-delete',
