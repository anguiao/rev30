...
describe('resource routes', () => {
  dbTest(
    'creates resources in the database and returns paginated resources',
    async ({ db: database }) => {
  ...
  dbTest(
    'limits non-admin resource mutations to the actor access scope',
    async ({ db: database }) => {
  ...
  dbTest(
    'creates typed menus, external links, and action permission points',
    async ({ db: database }) => {
      const app = await createTestApp(database)
  ...
  dbTest(
    'returns field errors when creating resources with invalid type-specific fields',
    async ({ db: database }) => {
      const app = await createTestApp(database)
  ...
  dbTest(
    'filters resource lists by keyword, type, status, and parent id',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: root } = await createResource(app, {
        type: RESOURCE_TYPE_DIRECTORY,
  ...
  dbTest('returns resource details and resource trees', async ({ db: database }) => {
    const app = await createTestApp(database)
    const { body: root } = await createResource(app, {
      type: RESOURCE_TYPE_DIRECTORY,
      name: 'System',
  ...
  dbTest(
    'returns lightweight resource tree options with enabled and non-deleted nodes by default',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: root } = await createResource(app, {
        type: RESOURCE_TYPE_DIRECTORY,
  ...
  dbTest(
    'includes disabled resources and required non-deleted ancestors when includeIds is provided',
    async ({ db: database }) => {
      const app = await createTestApp(database)
  ...
  dbTest(
    'updates resource fields, normalizes type-specific fields, and rejects circular moves',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: root } = await createResource(app, {
        type: RESOURCE_TYPE_DIRECTORY,
  ...
  dbTest(
    'defaults open target to blank when updating a resource to an external link',
    async ({ db: database }) => {
      const app = await createTestApp(database)
  ...
  dbTest(
    'defaults open target to self when updating an external link to an internal menu',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body } = await createResource(app, {
        type: RESOURCE_TYPE_EXTERNAL,
  ...
  dbTest('preserves external open target on partial updates', async ({ db: database }) => {
    const app = await createTestApp(database)
    const { body } = await createResource(app, {
      type: RESOURCE_TYPE_EXTERNAL,
      name: 'Docs',
  ...
  dbTest('rejects invalid final type fields on update', async ({ db: database }) => {
    const app = await createTestApp(database)
    const { body } = await createResource(app, {
      type: RESOURCE_TYPE_DIRECTORY,
      name: 'System',
  ...
  dbTest(
    'normalizes omitted-type update fields against the existing resource type',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: menu } = await createResource(app, {
        type: RESOURCE_TYPE_MENU,
  ...
  dbTest(
    'rejects invalid external urls when updating an existing external resource',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body } = await createResource(app, {
        type: RESOURCE_TYPE_EXTERNAL,
  ...
  dbTest('rejects duplicate resource codes on create and update', async ({ db: database }) => {
    const app = await createTestApp(database)
    await createResource(app, {
      type: RESOURCE_TYPE_DIRECTORY,
      name: 'System',
  ...
  dbTest('allows recreating a resource code after soft delete', async ({ db: database }) => {
    const app = await createTestApp(database)
  ...
  dbTest(
    'soft deletes empty resources and rejects deleting resources with children',
    async ({ db: database }) => {
      const app = await createTestApp(database)
      const { body: root } = await createResource(app, {
        type: RESOURCE_TYPE_DIRECTORY,
  ...
  dbTest('rejects deleting resources assigned to roles', async ({ db: database }) => {
    const app = await createTestApp(database)
  ...
  dbTest('returns siblings sorted by sortOrder in ascending order', async ({ db: database }) => {
    const app = await createTestApp(database)
    const { body: root } = await createResource(app, {
      type: RESOURCE_TYPE_DIRECTORY,
      name: 'System',
