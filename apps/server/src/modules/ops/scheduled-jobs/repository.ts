...
import { getNextCronOccurrences, parseCronSchedule, type CronSchedule } from '@rev30/utils'
import {
  and,
  asc,
  count,
...
import { ScheduledJobNotFoundError, ScheduledJobStateConflictError } from './errors'

export type RecoverableScheduledJobRun = {
  taskKey: ScheduledJobTaskKey
  scheduledFor: Date | null
  startedAt: Date
}

export type ScheduledJobRunCompletion =
  | {
      status: 'success'
      finishedAt: Date
      durationMs: number
...
function actorValues(actor: Pick<User, 'id' | 'nickname' | 'username'>) {
  return {
...
export function createScheduledJobRepository(database: Db) {
  return {
    async listDueScheduled(input: { now: Date }) {
    ...
    async updatePlan(input: { taskKey: string; schedule: CronSchedule; now: Date }) {
      return await database.transaction(async (tx) => {
        ...
        const nextRunAt = plan.enabled
          ? getNextCronOccurrences(input.schedule, input.now, 1)[0]!
          : null
        const [updated] = await tx
          .update(opsScheduledJobs)
          .set({
            cronExpression: input.schedule.expression,
    ...
    async initialize(input: { taskKeys: readonly ScheduledJobTaskKey[]; startupAt: Date }) {
      return await database.transaction(async (tx) => {
        const plans = await tx
          ...
          .orderBy(asc(opsScheduledJobs.taskKey))
          .for('update')
        const taskKeys = [...input.taskKeys].sort()
        const planKeys = plans.map(({ taskKey }) => taskKey).sort()

        if (
          taskKeys.length !== planKeys.length ||
          taskKeys.some((key, index) => key !== planKeys[index])
        ) {
          throw new Error('Scheduled job 任务集合与数据库计划集合不一致')
        }
        ...
        for (const run of runningRuns) {
          await tx
            .update(opsJobRuns)
            .set({ status: 'interrupted', finishedAt: input.startupAt })
        ...
        const latestRuns = await tx
          .selectDistinctOn([opsJobRuns.taskKey])
          .from(opsJobRuns)
          .where(and(inArray(opsJobRuns.taskKey, taskKeys), ne(opsJobRuns.status, 'skipped')))
          .orderBy(asc(opsJobRuns.taskKey), desc(opsJobRuns.createdAt), desc(opsJobRuns.id))

        const recoverableRuns = latestRuns
          .filter(
          ...
          .map((run): RecoverableScheduledJobRun => ({
        ...
        const interruptedRuns = runningRuns.map((run) => ({
          runId: run.id,
          taskKey: run.taskKey as ScheduledJobTaskKey,
          triggerSource: run.triggerSource as ScheduledJobTriggerSource,
        }))

        return { recoverableRuns, interruptedRuns }
      })
    },
    ...
    async claimScheduled(input: {
      ...
      return await database.transaction(async (tx) => {
        ...
        if (input.allowRunning === false) return null
        ...
        await tx
          ...
          .where(eq(opsScheduledJobs.taskKey, input.taskKey))
        return { runId, blockedByRunId: null, scheduledFor }
      })
    },
    ...
    async claimManual(input: {
      ...
      return await database.transaction(async (tx) => {
        ...
        await tx
          ...
          .where(eq(opsScheduledJobs.taskKey, plan.taskKey))
        return { runId, blockedByRunId: null, scheduledFor: null }
      })
    },

    async claimRecovery(input: { run: RecoverableScheduledJobRun; now: Date }) {
      return await database.transaction(async (tx) => {
        const plan = await lockPlan(tx, input.run.taskKey)
    ...
    async requestCancellation(input: {
      ...
      return await database.transaction(async (tx) => {
        ...
        if (run.cancelRequestedAt !== null) return run

        const [updated] = await tx
          .update(opsJobRuns)
          .set({
            cancelRequestedAt: input.now,
            cancelRequestedByUserId: input.actor.id,
            cancelRequestedByUsername: input.actor.username,
            cancelRequestedByNickname: input.actor.nickname,
          })
          .where(eq(opsJobRuns.id, run.id))
          .returning()
        return updated!
    ...
    async finalizeRun(input: {
      taskKey: ScheduledJobTaskKey
      runId: string
      completion: ScheduledJobRunCompletion
    }) {
      return await database.transaction(async (tx) => {
        const plan = await lockPlan(tx, input.taskKey)
        if (!plan) throw new ScheduledJobStateConflictError()
        ...
        const status = cancelled ? ('cancelled' as const) : input.completion.status
        const errorCategory =
          !cancelled && input.completion.status === 'failure'
            ? input.completion.errorCategory
            : null
        const errorSummary =
          !cancelled && input.completion.status === 'failure' ? input.completion.errorSummary : null

        if (run.status !== 'running') {
          const matches =
            plan.activeRunId !== run.id &&
            run.status === status &&
            sameDate(run.finishedAt, input.completion.finishedAt) &&
            run.durationMs === input.completion.durationMs &&
            run.deletedCount === input.completion.deletedCount &&
            run.failedCount === input.completion.failedCount &&
        ...
        const [updated] = await tx
          ...
          .set({
            status,
            finishedAt: input.completion.finishedAt,
            durationMs: input.completion.durationMs,
            deletedCount: input.completion.deletedCount,
            failedCount: input.completion.failedCount,
            errorCategory,
            errorSummary,
          })
          .where(eq(opsJobRuns.id, run.id))
        ...
        await tx
          .update(opsScheduledJobs)
          .set({ activeRunId: null, updatedAt: input.completion.finishedAt })
          .where(eq(opsScheduledJobs.taskKey, input.taskKey))
        return updated!
      })
    },
...
export type ScheduledJobRepository = ReturnType<typeof createScheduledJobRepository>
