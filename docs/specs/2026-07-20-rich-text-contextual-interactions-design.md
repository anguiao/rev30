...
本设计采用“分层上下文交互”，在保留顶部工具栏主入口地位的前提下，为不同 preset 增加 Quickbar 和块命令面板。自定义右键菜单不在范围内。
...
- 移动端专属底部工具栏或触屏专属交互。
- 业务方自定义 Quickbar、块命令、菜单组件、slot 或公开内部 DSL。
...
- **块命令面板**：用于块类型转换和块内容插入的命令列表，由 `+` 和 `/` 共同使用。
...
| CodeBlock Quickbar | 无 | 代码语言、转为正文 |
| 空块命令面板 | 无 | 空块 `+` 与 `/` 共用 |
| 自定义右键菜单 | 无 | 无 |
...
editor 禁用或上下文失效时不显示 Quickbar。主要输入设备匹配 `pointer: coarse` 的粗指针环境下隐藏全部 Quickbar，继续使用现有顶部工具栏；该规则不隐藏空段落 `+` 或 Slash command。本阶段不提供移动端专属替代界面。
...
`+` 与 `/` 使用相同的分组和命令顺序。查询过滤后隐藏没有命令的空分组，但不重排剩余命令；键盘上下导航跨分组连续移动，分组标题不可选中。
...
`正文` 作为默认块类型保留在命令列表中。通过 `/正文` 执行时，删除查询文本并保持段落类型；通过空段落 `+` 选择时，关闭面板并将焦点留在当前段落。

## 架构

### Preset 配置

`RichTextEditorPreset` 内部新增两个可选配置：
...
- `compactRichTextEditorPreset` 配置 `quickbar`，不配置 `blockMenu`。
- `createAllRichTextEditorPreset` 同时配置 `quickbar` 和 `blockMenu`。
- `defineRichTextEditorPreset` 校验 Quickbar control、Feature Quickbar 和块命令引用的 feature 已存在于 preset，且具有对应 editor implementation；同时拒绝 Quickbar 中重复的 feature 或 key。
...
Feature Quickbar component 由容器统一注入 `editor` 和 `disabled`，配置中的 `props` 只承载颜色、语言和上传回调等 feature-specific 选项。组件在配置阶段使用与现有 toolbar component control 相同的 `markRaw` 处理。

`RichTextEditor` 内部提供按 editor instance 隔离的临时操作层协调器，供 `RichTextQuickbar` 与所有顶部复杂 control 共享，不纳入 preset 或 public API。顶部 popover、dropdown 或 dialog 打开时注册 owner token，使唯一 BubbleMenu 不渲染任何 Quickbar 上下文；关闭或组件卸载时只注销自己的 token，最后一个顶部临时层注销后才从当前 selection 重新计算 Quickbar。
...
- 块命令直接通过 Tiptap chain 的 `.command(action.command(...arguments_))` 组合同一 action，不需要 block-menu-specific adapter 或额外的可组合 action 变体。
...
Toolbar、普通文字 Quickbar 和块命令 wrapper 可以引用同一个无参数 action item。需要 URL、颜色、语言等参数的 action 继续由 feature component 或 UI command 包装。Toolbar 的 button、dropdown、component control 仍保持 toolbar-specific；不新增自动注册的全局 surface registry。
...
- 顶部临时操作层完整包含：SearchReplace popover、Link 输入、Highlight 颜色面板、TextStyle 的文字颜色/字体/字号/行高面板、Heading/TextAlign/List 通用 dropdown、CodeBlock 语言列表和 ImageDialog。无论由顶部按钮还是现有 SearchReplace 快捷键打开，都必须注册协调器 owner。顶部普通 action button 不是临时操作层，不抑制 Quickbar。
- 打开顶部临时操作层前，当前 Quickbar 子界面先按“点击外部”关闭：未应用草稿丢弃，该次顶部触发产生的新焦点不被覆盖。随后协调器注册当前 owner，并隐藏整个 Quickbar；不会把 Quickbar 草稿传递给顶部 control。
- 同一 editor instance 任意时刻只允许一个顶部临时操作层。打开新的顶部临时层时，已有顶部临时层按点击外部规则关闭，再由新 owner 接管。协调器使用 owner token 避免旧组件的延迟关闭事件错误解除新 owner 的抑制状态。
- 顶部临时层的成功、显式取消、`Escape`、点击外部和目标失效仍分别遵循对应 feature 契约。所有关闭路径和组件卸载都必须注销 owner；没有 active owner 后按当前 selection 重新计算 Quickbar，不无条件恢复旧上下文。
...
- 顶部 Link 输入按“顶部临时操作层协调契约”接管并抑制 Quickbar。Quickbar 向顶部输入转移时不传递未应用草稿，顶部输入从原始文档值重新初始化。
...
- 从顶部工具栏打开的 ImageDialog 按“顶部临时操作层协调契约”抑制 Quickbar。从 Image Quickbar 打开时，对话框作为当前 Quickbar 层的子界面替代可见浮条，关闭后再按 selection 重新计算；从 `+` 或 `/图片` 打开前先关闭块命令面板。后三种来源都不注册为顶部 owner。
...
| CodeBlock Quickbar | 切换语言；“转为正文” | 仅作为已有 CodeBlock 的自动上下文界面 |
...
- 顶部语言列表按“顶部临时操作层协调契约”抑制 Quickbar；Quickbar 内的语言列表属于当前 Quickbar 层。
- 在已有 CodeBlock 上，顶部 active 代码块按钮与 Quickbar“转为正文”复用同一个 `codeBlockAction`。执行后聚焦 editor，并使用 Tiptap 映射后的对应 selection；不另行推测光标位置。
...
- 顶部颜色面板按“顶部临时操作层协调契约”抑制 Quickbar；Quickbar 内的颜色面板属于当前 Quickbar 层。
...
├── block-menu.ts
└── block-menu/
    ├── RichTextBlockMenu.vue
    └── RichTextBlockCommandList.vue
```

- `block-menu.ts` 定义命令分组、关键词、action/UI command wrapper 和内部 helper。
- `RichTextBlockMenu.vue` 使用 Tiptap Vue `FloatingMenu` 渲染空块 `+`，并负责打开命令面板。
- `RichTextBlockCommandList.vue` 负责分组、查询过滤、active option、键盘导航和命令选择。
- `/` 入口使用 Tiptap Suggestion utility，并挂载同一个 `RichTextBlockCommandList`。
- 两个入口共用同一份 preset 命令配置、过滤规则和执行逻辑，不维护两套列表。

新增 editor-only `block-command` feature，仅由 `allRichTextPreset` 启用。其 editor implementation 安装 `/` Suggestion 所需扩展；它不声明 server implementation，也不贡献文档 extension。新增直接依赖 `@tiptap/suggestion`，版本与项目内其它 Tiptap 依赖保持一致。

使用互不冲突的内部 plugin key：

- Quickbar：`richTextQuickbar`。
- 空块 FloatingMenu：`richTextBlockMenu`。
- Slash Suggestion：`richTextSlashCommand`。

#### 块命令列表共享契约

- `+` 与 Slash 挂载同一个 `RichTextBlockCommandList`，使用同一份命令分组、固定顺序、标签、图标、关键词、过滤函数和执行 wrapper。`+` 使用空查询并展示全部命令；Slash 使用当前 `/query` 过滤。
...
- Slash 的 `/query` 因直接输入、Backspace 或 composition 结束而变化时，不保留旧 active option；每次都重置为新过滤结果中的第一个 enabled 命令，没有 enabled 结果时清空 active option。`+` 没有查询变化，因此不需要该重置。
- 点击 enabled 命令或在存在 active option 时按 `Enter`，都通过同一个选择入口执行。action 返回 false 时保持面板打开，并从下一次 transaction 重新计算 enabled 状态。
- `Tab` 在两个入口中都关闭面板、保留来源内容且不消费事件，由浏览器继续默认焦点导航。`+` 保留来源空段落；Slash 保留已输入的 `/query`。
- 两个入口的焦点归属不同：`+` 由 listbox 持有焦点；Slash 由 editor 持有焦点。没有 active option 时，`+` 的 `Enter` 不执行并保持面板打开，Slash 的 `Enter` 交给 editor 正常换行。其它输入、composition 与关闭差异由各入口章节定义。
- 普通同步命令复用同一个 action command，但 transaction 组装不同：`+` 直接执行 action；Slash 先在同一个 chain 中删除 `/query`，再组合 action。图片等 UI command 复用同一个 wrapper，并由入口提供各自插入锚点。
- 图片对话框属于异步 UI 边界，不尝试跨对话框生命周期复用或合并 transaction。`/图片` 在选择命令时删除 `/query`，确认时另用一个 transaction 插入图片；`+` 没有查询删除，只在确认时产生图片插入 transaction。
...
- CodeBlock Quickbar 与顶部 CodeBlockToolbarControl 复用目标解析器、语言选项、`setCodeBlockLanguageAction` 和现有 code block toggle action，具体状态与 selection 结果只按“CodeBlock 共享交互契约”实现。
- Heading、List、Blockquote、CodeBlock、HorizontalRule 的块命令引用现有 action；图片命令属于 UI command，打开现有 `ImageDialog`。
...
- Quickbar 中按 `Escape` 时关闭当前子界面，恢复原 editor selection 与焦点。Link URL 输入态改用表单的正常 Tab 顺序，不拦截输入框的左右方向键；退回只读 toolbar 态后恢复 roving tabindex。“更多”和高亮颜色面板打开时将焦点移入子菜单，关闭后回到各自触发按钮。
...
- 仅在 editor 可编辑、获得焦点且当前为顶层空段落时显示。
- `+` 位于空段落左侧，点击后打开完整块命令面板。粗指针环境下仍显示同一入口，不增加触屏专属变体。
- `+` 使用至少 `44×44px` 的点击目标。EditorContent 在内部预留左侧 gutter，按钮定位在编辑区可见范围内，不放到会被 `RichTextEditor` 现有 `overflow-hidden` 裁切的外部区域。
- `+` 按钮使用 `aria-haspopup="listbox"` 和 `aria-expanded`。打开面板后将焦点移到可聚焦的 listbox，由 listbox 通过 `aria-activedescendant` 和键盘事件管理 active option。面板打开后固定当前空段落锚点，不因 editor 在焦点移入 listbox 时短暂失焦而自动关闭；关闭或锚点失效时解除固定。
- 按 `Escape` 关闭 `+` 面板时，将焦点恢复到原空段落。点击面板外部时关闭面板，但保留该次点击产生的新焦点。按 `Tab` 时关闭面板、保留来源空段落且不消费事件，由浏览器继续默认焦点导航。
- 命令的 active option、disabled、方向键、`Enter`、点击和执行失败行为统一按“块命令列表共享契约”处理。
- 选择普通命令后执行 action 并恢复 editor 焦点。
- 选择图片后保存当前顶层空段落位置作为插入锚点，关闭命令面板并将焦点交给现有图片对话框。
...
- 命令的 active option、disabled、方向键、`Enter`、点击和执行失败行为统一按“块命令列表共享契约”处理。所有匹配命令均 disabled 时不设置 active option，`Enter` 交给 editor 正常处理。
- 执行同步命令时，先创建包含 `deleteRange(/query)` 的 chain，再通过 `.command(action.command(...arguments_))` 组合块操作，最后只调用一次 `run()`。删除和 action 必须处于严格的同一个 transaction，而不是依赖 history 的时间窗口将两个 transaction 合并。这使 Undo 恢复为一次操作，也避免 `update:modelValue` 观察到“查询文本已删除、块命令尚未执行”的中间状态。
- Slash 命令的 enabled 状态使用 `editor.can().chain().deleteRange(/query).command(action.command(...arguments_)).run()` 对完整操作做无 dispatch 模拟，不只在删除前的 editor state 上调用通用 `canRunRichTextAction`。`+` 入口没有 query range，直接从 action command 派生 enabled 状态。
- 执行图片 UI command 时，先删除 `/query`，再保存删除后留下的顶层空段落位置作为插入锚点，关闭命令面板并打开对话框。
- 来自 `+` 和 `/图片` 的对话框确认时，恢复并验证保存的插入锚点；只有该位置仍是原顶层空段落时才在此插入图片。对话框期间 selection 变化不改变插入目标；外部内容更新导致锚点失效或该段落不再为空时，关闭对话框且不插入、不推测替代位置。
- 图片插入成功后，共享 `insertImageAction` 选中结果图片并聚焦 editor，不创建后继段落、不把光标移入已有后继段落；其它成功状态按“Image 共享交互契约”处理。
- 取消来自 `+` 或 `/图片` 的对话框时保留来源空段落。
...
- 图片处理错误继续走 preset 已提供的 `onError`；Quickbar 和块命令面板不引入第二套错误通道。
- Quickbar 和块命令面板自身不发起服务端请求，不增加通用 `try/catch` 或 fallback。
...
- 命令列表使用 listbox/option 语义，通过 `aria-activedescendant` 关联 active option。Slash 入口由 editor 持有焦点和 ARIA 关联，`+` 入口由可聚焦 listbox 持有焦点和 ARIA 关联。
- Link 输入、颜色面板、语言列表、图片对话框和命令列表均支持 `Escape` 关闭，并按交互来源处理焦点；由取消或 `Escape` 关闭时恢复 editor 焦点，由点击外部关闭时保留点击产生的新焦点。顶部临时操作层还必须在任何关闭路径注销协调器 owner。
- BubbleMenu 和 FloatingMenu 使用 Tiptap/Floating UI 的 placement、offset 和 flip 能力，避免遮挡当前选区并适应编辑器边缘。
...
- 不公开 Quickbar、block menu config、内部 helper 或自定义组件入口。
...
- 校验 Quickbar、Feature Quickbar 和块命令不能引用 preset 未启用的 feature。
- 校验 `compact` 不包含 block-command feature 或 block menu config。
- 校验 `all` 包含约定的 Quickbar 上下文、命令分组和 block-command editor feature。
...
- 验证 Quickbar 与 SearchReplace、Link、Highlight、TextStyle、Heading、TextAlign、List、CodeBlock 和 Image 的全部顶部 popover/dropdown/dialog 互斥；SearchReplace 由按钮或现有快捷键打开时结果相同。顶部临时层打开前按点击外部规则关闭 Quickbar 子界面并放弃未应用草稿，打开期间不渲染任何 Quickbar，关闭后按当前 selection 恢复。验证顶部临时层之间也互斥，旧 owner 的延迟关闭或组件卸载不会清除新 owner 的抑制状态。
...
- 验证顶部工具栏、Image Quickbar、`+` 和 `/图片` 共享图片状态机：插入或编辑成功后都形成目标图片的 Image NodeSelection 并聚焦 editor，不创建后继段落、不移动到已有后继段落；允许显示 Quickbar 时重新进入 Image Quickbar。验证每次 insert/update action 自身在一个 transaction 中完成内容与 selection 变更且可单步 Undo；`/图片` 的查询删除按块命令测试中的独立 History 规则验证。
- 验证 CodeBlock 读取当前语言、切换语言和转为正文。
- 验证折叠光标和非空选区完全位于单个 CodeBlock 时，顶部与 Quickbar 的语言控件都可用并指向同一块；跨 CodeBlock 或跨其它块时，顶部语言控件 disabled 且 CodeBlock Quickbar 不显示。验证切换后保留原 selection 和焦点，目标失效时不修改其它块。
...
- 验证 `+` 只在可编辑的顶层空段落显示，但不因 `pointer: coarse` 隐藏；点击目标至少为 `44×44px` 且不被 editor 容器裁切。
- 验证 `+` 打开后焦点进入 listbox，`Escape` 恢复原空段落焦点，点击外部则保留新焦点；`Tab` 关闭并保留空段落后继续浏览器默认焦点导航。Slash 打开期间 editor 保持焦点并具有正确的 ARIA 关联。
...
- 验证两个入口共享 disabled、初始 active option、循环导航、点击和执行失败行为；没有 active option 时 `+` 的 `Enter` 保持面板，Slash 的 `Enter` 交给 editor。验证 Slash 的 enabled 状态从包含 query 删除的完整模拟 chain 计算。
...
- `compact` 只增加普通文字与 Link Quickbar，不加载 block-command 相关 extension。
- `all` 增加普通文字、Link、Image、CodeBlock Quickbar，以及共享命令配置的 `+` 和 `/`。
- 任意时刻最多显示一个 Quickbar 和一个块命令面板；任意顶部工具栏临时操作层与 Quickbar 绝不同时显示，且同一 editor instance 最多打开一个顶部临时操作层。
...
- 新增内部 `vue/quickbar*`、`vue/block-menu*` 模块。
...
- 新增 editor-only `block-command` feature。
