...
| 前端刷新 | 仅对用户正在关注的单个运行每 2 秒轮询，终态或离开后停止 |
...
- 启用状态。
- 下次执行时间；禁用时显示“已禁用”。
- 当前状态；`running + cancelRequestedAt` 显示“取消中”。
- 最近终态及结束时间。
- 按权限显示编辑、启停、立即执行、取消和查看日志。

页面初次进入和用户操作成功后刷新列表，并提供显式刷新。页面不持续轮询全部任务。
...
影响当前运行；重新启用不会补跑禁用期间的计划。

### 手动执行与取消

手动执行成功后显示已提交状态，保存 `runId` 并每 2 秒查询该单条详情；到达任一终态立即
停止。`409 overlap` 显示当前已有运行并刷新任务行，不自动重试。

取消按钮只对当前 `running` 且拥有取消权限的用户显示。请求接受后显示“取消中”，继续轮询
同一运行直到终态；不承诺立即停止当前数据库或存储调用。
...
取消 timer 和未完成查询。状态中文映射和操作日志 action 中文标签集中放在 ops feature
labels，不在页面内重复散落。
