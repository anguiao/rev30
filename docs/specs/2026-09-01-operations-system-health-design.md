...
2. 一个“运行概况”容器，内部按当前实例、数据库、附件存储和任务调度分成四组指标，不另建四张卡片。实例显示 RSS、Heap 已用和运行时间；数据库与附件存储分别显示状态和检查耗时；任务调度显示运行状态、自动执行数与并发上限，以及来自数据库共享状态的运行中任务和延迟启动计数。“延迟启动”是明显积压规则的页面文案，仍以启用任务超过计划时刻 60 秒为判断条件。
3. 诊断详情入口位于“运行概况”容器右上角，支持键盘操作，在同一容器内按需展开。详情分为“运行环境”“数据库与存储检查”和“任务启动与恢复”，三组各展示四行字段，保持相近的信息密度。仅展示字段名、状态与数值，不添加常驻说明段落；“延迟启动”的判断条件及存在延迟任务时的最早计划时间放在对应字段的悬停提示中。概览与趋势之间不插入独立说明段落或折叠块。
4. 最近 10 分钟 RSS、数据库延迟和 storage 延迟折线图，紧随当前概览与诊断。
5. 任务运行统计区域，先展示最近 5 条异常任务及运行日志入口，再展示历史统计图；无异常时使用紧凑文字空状态。
6. 历史统计包含 7 日任务运行结果堆叠柱状图、30 日状态分布环图、30 日失败分类环图，以及成功任务平均耗时 Top 5 横条排行。

字节复用现有 `bytes` 格式化能力，耗时明确标注单位，快照、探测及统计时间显示到秒。`nodeVersion`、platform 和 arch 放在实例详情区域，不占用顶部主指标卡。

诊断字段使用可理解的名称：数据库与附件存储分别展示检查时间，存储结果来源区分本次检查与最近 30 秒内的检查。任务调度的 `manualStarting` 单独展示为“手动启动处理中”的请求数，`recoveryQueued` 单独展示为“重启后待恢复任务”的数量；`lastPollAt` 与 `nextWakeAt` 分别展示为最近与下次调度检查时间，不承诺下次任务执行时间。`retryPending` 为 true 时，在下次调度检查时间后附加“重试”状态，不单独占一行。页面不使用“认领”“自动槽”“排队”“退避”等内部实现术语，也不通过说明段落解释调度器内部流程。

任务统计图与最近异常放在同一区域，共用统计时间 `generatedAt`、加载状态和刷新错误提示，避免把顶部快照时间误认为所有区域的数据时间；同时固定展示“按当前留存日志统计，已清理记录不计入统计”和上海统计时区，不把固定查询窗口描述为完整历史覆盖。快照与统计区域分别标注 10 秒和 60 秒的自动刷新周期。
...
- Bar、Line、Pie charts。
- Grid、Tooltip、Legend、Title 等实际使用组件。
- Canvas renderer。
...
图表使用纯色，不启用 ECharts decal 花纹；保留 ARIA 文字描述。任务状态按语义显式配色，堆叠柱图与状态环图使用相同映射，成功使用绿色，失败与中断使用可区分的红色系，不把中断映射到成功色。主题色、文字色与分隔线沿用 Naive UI 主题。

每张图都有可见标题、数值摘要和无数据状态，不把 canvas 作为唯一信息来源。多状态或多分类图保留图例，趋势等单序列图省去重复图例。环图中央显示总次数；状态与分类摘要只列非零项，图表数据仍保留全部状态及分类。各空状态明确对应时间窗与留存范围，失败分类为空时显示“近 30 日留存日志中无失败任务”。趋势的采样与断点规则在区域内统一说明，各图摘要仅展示最新数值、样本时间与有效样本数。

耗时排行使用 Vue ECharts 横向条形图，左侧显示任务名，右侧对齐显示平均耗时和成功运行次数，信息直接整合在图内。省去数值轴刻度、轴线、网格及图下重复明细，保留悬停交互与包含完整数据的 ARIA 文字描述；横条使用项目主题色。全部耗时为 0 时仍展示实际数值，无样本时保留明确的空状态。

RSS 趋势按 MB 绘制，换算口径与 `bytes` 一致（1 MB = 1024² 字节）；纵轴统一标注 MB，自动生成整数刻度，最小间隔为 1 MB，不把原始字节刻度转换成带小数的标签。换算保留采样精度与断点，悬停提示和数值摘要仍按原始字节值格式化。数据库与附件存储延迟以 ms 绘制，纵轴使用整数刻度，最小间隔为 1 ms，两张图的范围仍各自自适应。
