...
pnpm --filter @rev30/contracts test __tests__/schemas/auth.test.ts
pnpm --filter @rev30/contracts coverage
```

覆盖率只用于确认公开 schema 入口和自定义规则是否存在盲区，不设置百分比门槛。HTML 报告生成在 `packages/contracts/coverage/`。
