...
import { tableHtmlPolicy } from '../../../../src/features/table/server/feature'
...
describe('table server feature', () => {
  ...
  it('normalizes the table wrapper and keeps only dynamic table attributes', () => {
    const html = sanitizeRichTextHtml(
      '<div class="evil" style="overflow-x: scroll; color: red" tabindex="9" role="button" onclick="alert(1)"><table style="width: 120px; border: 99px solid red; border-collapse: separate; color: red" data-table="evil"><tbody><tr><td colspan="0" rowspan="2" colwidth="20,30" style="min-width: 1px; border: 99px solid red; padding: 9rem; text-align: center; vertical-align: bottom; color: red">内容</td></tr></tbody></table></div>',
      [tableHtmlPolicy],
    )

    expect(html).toContain(
      '<div class="tableWrapper" tabindex="0" role="region" aria-label="可横向滚动的表格">',
    )
    expect(html).toContain('<table style="width:120px">')
    expect(html).toContain('<td rowspan="2" colwidth="20,30" style="text-align:center">内容</td>')
