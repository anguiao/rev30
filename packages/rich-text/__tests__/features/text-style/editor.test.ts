...
describe('text style editor feature', () => {
  ...
  it('combines and removes the four styles on one span', () => {
    ...
    expect(setTextColorAction.canRun?.(editor, '#ef4444')).toBe(true)
    expect(setTextColorAction.run(editor, '#ef4444')).toBe(true)
    expect(setFontFamilyAction.run(editor, 'serif')).toBe(true)
    expect(setFontSizeAction.run(editor, '14pt')).toBe(true)
    expect(setLineHeightAction.run(editor, '1.5')).toBe(true)
    ...
    expect(unsetTextColorAction.canRun?.(editor)).toBe(true)
    expect(unsetTextColorAction.run(editor)).toBe(true)
    expect(unsetFontFamilyAction.run(editor)).toBe(true)
    expect(unsetFontSizeAction.run(editor)).toBe(true)
    expect(unsetLineHeightAction.run(editor)).toBe(true)
    expect(editor.getJSON()).toMatchObject({
